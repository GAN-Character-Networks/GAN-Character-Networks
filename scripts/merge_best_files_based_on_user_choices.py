...
def create_filtered_csv(file1, file2, json_file, output_file):
    ...
    with open(file1, newline="") as csvfile1, open(
        file2, newline=""
    ) as csvfile2, open(output_file, "w", newline="") as output_csv:
...
def main():
    file1 = (
        "../submission_058.csv"  # Remplace by the path to the first CSV file
    )
    file2 = (
        "../submission_070.csv"  # Remplace by the path to the second CSV file
    )
