...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        indices = self.tokenizer.convert_tokens_to_ids(
...
def collate_fn(batch):
    ...
    attention_mask = torch.where(
        padded_inputs != 1, torch.tensor(1), torch.tensor(0)
    )
...
def create_train_test_dataset(csv_file, tokenizer, batch_size=32):
    ...
    train_data = [
        (ast.literal_eval(tokens), ast.literal_eval(labels))
    ...
    valid_data = [
        (ast.literal_eval(tokens), ast.literal_eval(labels))
        for tokens, labels in valid_data
    ]
...
def train(model, valid_dataloader, train_dataloader, epochs, optimizer, device):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            outputs = model(
                tokens, labels=labels, attention_mask=attention_mask
            )
...
def evaluate(model, dataloader, device):
    ...
    print(f"Accuracy: {accuracy}, Loss: {total_loss / len(test_dataloader)}")
    return accuracy
...
if "__main__" == __name__:
    ...
    (
        ...
        "data/finetuning_data/train_data.csv", tokenizer, batch_size=batch_size,
    )
    ...
    print("Training........")
    loss_array = train(
        model, valid_dataloader, train_dataloader, 10, optimizer, device
    )
